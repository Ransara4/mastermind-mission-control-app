...
export function useOnlineProgramData() {
  ...
  const fetchPosts = useCallback(async (filters: BlogFilters = {}) => {
    try {
      ...
      const res = await fetch(`/api/online-program/blog${qs ? `?${qs}` : ""}`);
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch("/api/online-program/stats");
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const res = await fetch("/api/online-program/settings");
  ...
  const rejectPost = useCallback(async (id: number, reason: string) => {
    const res = await fetch("/api/online-program/blog", {
      method: "PUT",
  ...
  const publishPost = useCallback(async (id: number) => {
    const res = await fetch("/api/online-program/publish", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const triggerGeneration = useCallback(async () => {
    const res = await fetch("/api/online-program/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    });
