...
export default function VillaSearchPage() {
  ...
  useEffect(() => {
    try {
      const s = localStorage.getItem("villa_status");
      const n = localStorage.getItem("villa_notes");
  ...
  const setStatus = (id: string, val: string) => {
    ...
    setStatusMap(next);
    localStorage.setItem("villa_status", JSON.stringify(next));
  };

  const setNote = (id: string, val: string) => {
    const next = { ...notesMap, [id]: val };
    setNotesMap(next);
    localStorage.setItem("villa_notes", JSON.stringify(next));
  };
      ...
      <div className="bg-dark-panel border border-dark-border rounded-xl p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold tracking-tight text-dark-text">Villa Search DB</h1>
