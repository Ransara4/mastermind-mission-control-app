...
import { useBlogServiceData } from "@/hooks/useBlogServiceData";
import type { BlogServiceClient, BlogServicePost, WeeklyChartDay, ContentQuality } from "@/hooks/useBlogServiceData";
  ...
  return (
    <div className="bg-dark-panel rounded-xl border border-dark-border p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold  text-dark-text">Content Quality</h2>
        {scoreImproving && (
...
function WeeklyChart({ chart }: { chart: WeeklyChartDay[] }) {
  ...
  return (
    <div className="bg-dark-panel rounded-xl border border-dark-border p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold  text-dark-text">Posts This Week</h2>
...
function RecentPostsTable({ posts }: { posts: BlogServicePost[] }) {
  if (posts.length === 0) {
    return (
      <div className="bg-dark-panel rounded-xl border border-dark-border">
        <div className="p-5 border-b border-dark-border">
          <h2 className="text-base font-semibold  text-dark-text">Recent Posts</h2>
        </div>
                  ...
                  <td className="px-5 py-3">
                    {post.url ? (
                      <a
                        ...
                        className="text-cm-purple hover:text-cm-purple-mid transition-colors"
                      >
...
function RunNowButton({ onRun }: { onRun: () => Promise<void> }) {
  ...
  return (
    <div className="relative">
      ...
      {toast && (
        <div className="absolute right-0 top-full mt-1.5 flex items-center gap-1.5 bg-dark-success text-dark-bg text-xs font-medium px-3 py-1.5 rounded-lg shadow-sm whitespace-nowrap z-10">
          <CheckCircle size={12} />
          Run triggered
        </div>
      )}
...
export default function BlogServicePage() {
  const { data, loading, error, refresh, runNow, toggleClient } = useBlogServiceData();
