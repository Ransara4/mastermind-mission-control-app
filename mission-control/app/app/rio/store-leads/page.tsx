...
export default function StoreLeadsPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [statusRes, criteriaRes, storesRes, icpRes] = await Promise.all([
        fetch("/api/rio/store-leads?action=status"),
        fetch("/api/rio/store-leads?action=criteria"),
        fetch(`/api/rio/store-leads?action=stores${filterCountry ? `&country=${filterCountry}` : ""}${filterWhatsappOnly ? "&whatsapp_only=1" : ""}`),
        fetch("/api/rio/icp?file=shopify-store-owners"),
      ]);
  ...
  async function handleIcpSave(groupKey: string, value: string) {
    const res = await fetch("/api/rio/icp?file=shopify-store-owners", {
      method: "PUT",
  ...
  async function handleSync() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/rio/store-leads?action=sync", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({}) });
  ...
  async function handleExport() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/rio/store-leads?action=export-sheets", { method: "POST" });
      const data = await res.json();
      if (data.url) {
  ...
  async function handleAddCriteria() {
    ...
    const res = await fetch("/api/rio/store-leads?action=criteria", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleDeleteCriteria(id: number) {
    await fetch(`/api/rio/store-leads?action=criteria&id=${id}`, { method: "DELETE" });
    loadAll();
    showToast("Criteria deleted");
  }

  async function handleDisqualify(id: number) {
    await fetch(`/api/rio/store-leads?action=store&id=${id}`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
    ...
    <div className="max-w-6xl space-y-6">
      ...
      {agentStatus && !agentStatus.hasApiKey && (
        <div className="p-4 bg-dark-warn/10 border border-dark-warn/30 rounded-xl text-dark-warn text-sm">
          ...
          <code className="bg-dark-warn/20 px-1 rounded">~/.openclaw/workspace/.env</code> to enable syncing.
        </div>
      )}
          ...
          <div className="px-5 pb-5 space-y-5 border-t border-dark-border">
            ...
            <div className="mt-4 space-y-2">
              <div className="flex items-center justify-between mb-2">
                ...
                <Link
                  href="/app/rio/icps/shopify-store-owners"
