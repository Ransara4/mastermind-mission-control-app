...
export default function DiscoveriesPage() {
  ...
  const fetchDiscoveries = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/online-program/discoveries?${params.toString()}`
      );
  ...
  const fetchKeywords = useCallback(async () => {
    try {
      const res = await fetch(
        "/api/online-program/discoveries?type=keywords"
      );
      const data = await res.json();
  ...
  async function deleteDiscovery(id: number) {
    setDeletingId(id);
    try {
      await fetch("/api/online-program/discoveries", {
        method: "DELETE",
  ...
  async function runDiscovery() {
    setDiscovering(true);
    try {
      await fetch("/api/online-program/discover", { method: "POST" });
  ...
  const fetchInstructions = useCallback(async () => {
    try {
      const res = await fetch("/api/online-program/discovery-instructions");
      const data = await res.json();
  ...
  async function saveInstructions() {
    ...
    try {
      await fetch("/api/online-program/discovery-instructions", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
          ...
          <div className="p-4">
            ...
            <p className="text-xs text-dark-muted mt-2">
              ...
              <code className="bg-dark-panel2 px-1.5 py-0.5 rounded text-dark-muted">
                ~/.attache/workspace/projects/online-program/DISCOVERY_INSTRUCTIONS.md
              </code>
            </p>
          </div>
        </div>
