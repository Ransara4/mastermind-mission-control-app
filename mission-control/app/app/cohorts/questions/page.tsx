...
function QuestionDetailModal({
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const res = await fetch("/api/cohorts/questions/regenerate", {
        method: "POST",
...
export default function QuestionsPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [qRes, pRes] = await Promise.all([
        fetch("/api/cohorts/questions"),
        fetch("/api/cohorts/participants"),
      ]);
  ...
  const handleSave = async (data: {
    ...
    const res = await fetch("/api/cohorts/questions", {
      method,
      headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id: number) => {
    const res = await fetch(`/api/cohorts/questions?id=${id}`, { method: "DELETE" });
    if (!res.ok) throw new Error(await res.text());
  ...
  const toggleField = async (
    ...
    await fetch("/api/cohorts/questions", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const handleGenerateAnswers = async () => {
    ...
    try {
      const res = await fetch("/api/cohorts/questions/generate-answers", {
        method: "POST",
      });
      const data = await res.json();
