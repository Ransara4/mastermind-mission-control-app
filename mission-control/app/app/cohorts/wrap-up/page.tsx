...
interface SessionData {
  ...
  instagram_clips: string | null;
  approved_clips: string | null; // JSON array of approved clip filenames
...
interface StageDefinition {
  ...
  name: string;
  statusKeys: string[];
  type: "automated" | "manual" | "approval" | "skipped";
...
const STAGES: StageDefinition[] = [
  ...
  { stageNumber: 22, name: "Zoom Transcript",                  statusKeys: ["step_4_transcript_pasted"],       type: "automated" },
  { stageNumber: 4,  name: "Analyze Chat",                     statusKeys: ["step_5_chat_analyzed"],           type: "automated", runStep: 4 },
  { stageNumber: 20, name: "Save Unanswered Questions to DB",  statusKeys: ["step_4c_questions_saved"],        type: "automated", runStep: 20 },
  ...
  { stageNumber: 21, name: "Add Video to Workshop Guide",      statusKeys: ["step_2b_workshop_video"],         type: "automated", runStep: "2b" },
  { stageNumber: 3,  name: "Descript Import",                  statusKeys: ["step_3_descript_manual"],         type: "automated", runStep: 3 },
  { stageNumber: 23, name: "Instagram Clips",                  statusKeys: ["step_23_instagram_clips"],        type: "manual",    alwaysAccessible: true },
  { stageNumber: 24, name: "Post to Instagram on a Schedule",  statusKeys: ["step_24_instagram_post"],         type: "skipped" },
  // --- Review & Content ---
  { stageNumber: 6,  name: "Editorial Review",                 statusKeys: ["step_7_mc_reviewed"],             type: "approval",  alwaysAccessible: true },
  ...
  { stageNumber: 17, name: "Tech Requirements PDF",            statusKeys: ["step_15c_tech_pdf_built"],        type: "skipped" },
...
function isStageComplete(stage: StageDefinition, pipeline: PipelineStatus): boolean {
  if (stage.type === "skipped") return true;
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-dark-panel rounded-2xl shadow-2xl shadow-black/40 w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-dark-border">
          <h2 className="text-lg font-bold tracking-tight text-dark-text">New Mastermind Session</h2>
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            ...
            <input
              ref={titleRef}
              type="text"
              placeholder="e.g. Cohort 1, Session 2: Business Automation Masterminds"
...
function SessionPanel({
  ...
  const instagramClipsRef = useRef<HTMLTextAreaElement>(null);
  const dataLoadedRef = useRef(false);
  const pendingScrollY = useRef<number | null>(null);

  // Restore scroll position synchronously after any render that may remount children
  useLayoutEffect(() => {
  ...
  // Instagram clip list state
  const [clipList, setClipList] = useState<Array<{ file: string; path: string; status: "pending" | "queued" | "posted" }>>([]);
  const [clipListLoading, setClipListLoading] = useState(false);
  const [clipListLoaded, setClipListLoaded] = useState(false);
  const [openingClipsFolder, setOpeningClipsFolder] = useState(false);
  const [openFolderStatus, setOpenFolderStatus] = useState<"idle" | "success" | "error">("idle");

  const fetchClipList = useCallback(async (sNum: number) => {
    setClipListLoading(true);
    try {
      const res = await fetch(`/api/cohorts/wrap-up/clips-list?session=${sNum}`);
      if (res.ok) {
        const data = await res.json();
        setClipList(data.clips || []);
        setClipListLoaded(true);
      }
    } catch { /* non-critical */ }
    finally { setClipListLoading(false); }
  }, []);
  ...
  const fetchSession = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/cohorts/wrap-up?session=${sessionNum}`);
      if (!res.ok) {
        const body = await res.json().catch(() => ({ error: "Unknown error" }));
        throw new Error(body.error || `HTTP ${res.status}`);
      }
      ...
      try {
        ...
        const zoomRes = await fetch(`/api/cohorts/wrap-up/zoom-recording?session=${data.data.session_number}&cohort=${cohortNum}`);
  ...
  useEffect(() => {
    ...
    if (instagramClipsRef.current) instagramClipsRef.current.value = sessionData.instagram_clips || "";
  ...
  async function runStep(step: number | string): Promise<RunStepResult | null> {
    try {
      ...
      if (String(step) === "15b") {
        const res = await fetch("/api/cohorts/wrap-up/generate-tech-output", {
      ...
      const res = await fetch("/api/cohorts/wrap-up/run", {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  function Badge({ label, color }: { label: string; color: "amber" | "green" | "indigo" | "muted" }) {
    const c = color === "amber" ? "bg-dark-warn/10 text-dark-warn border-dark-warn/30" : color === "green" ? "bg-dark-success/10 text-dark-success border-dark-success/30" : color === "muted" ? "bg-dark-panel2 text-dark-muted border-dark-border" : "bg-cm-purple/10 text-cm-purple border-cm-purple/30";
    ...
    return (
      <div className={`bg-dark-panel border border-dark-border rounded-xl border-l-4 ${bc} ${sc} ${state === "locked" ? "opacity-60" : ""}`}>
        ...
        <div role="button" tabIndex={0} onClick={(e) => toggleStage(stage.stageNumber, e)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggleStage(stage.stageNumber); } }} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-dark-bg transition-colors text-left cursor-pointer select-none">
          ...
          {stage.type === "skipped" && <Badge label="Not Needed" color="muted" />}
          {state === "done" && stage.type !== "skipped" && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-dark-success/20 text-dark-success">Done</span>}
          {state === "done" && stage.type !== "skipped" && <CheckCircle2 size={18} className="text-dark-success shrink-0" />}
  ...
  function StageBody({ stage }: { stage: StageDefinition }) {
    switch (stage.stageNumber) {
      ...
      case 22: return (<div className="space-y-4"><div className="flex items-start gap-2 bg-dark-panel2 border border-dark-border rounded-lg px-4 py-3 text-sm text-dark-muted"><CheckCircle2 size={15} className="text-dark-success shrink-0 mt-0.5" /><span><span className="font-medium text-dark-text">Auto-downloaded alongside the Zoom recording.</span> The CC closed-caption file is fetched and converted to <span className="font-mono text-xs bg-dark-panel px-1.5 py-0.5 rounded">transcript-raw.md</span> as part of Step 1.</span></div><div><label className="block text-sm font-medium text-dark-text mb-1">Transcript <span className="text-xs font-normal text-dark-muted">{transcript ? `(${transcript.split("\n").length} lines — edit if needed)` : "(not yet downloaded)"}</span></label><textarea ref={transcriptRef} defaultValue={transcript} rows={18} className="w-full font-mono text-sm border border-dark-border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cm-purple focus:border-transparent resize-y text-dark-text bg-dark-panel2" placeholder="Transcript will appear here after Step 1 runs..." /></div><div className="flex items-center gap-3"><button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); patchSession({ transcript: transcriptRef.current?.value || "", pipeline_status: { step_4_transcript_pasted: true } }, "stage22"); }} disabled={saving} className="flex items-center gap-2 px-3 py-1.5 bg-dark-panel2 text-dark-text rounded-lg hover:bg-dark-panel2 disabled:opacity-50 text-sm font-medium transition-colors">{saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} Save edits</button><SaveIndicator statusKey="stage22" /></div></div>);
      case 3: return (<div className="space-y-4"><p className="text-sm text-dark-muted">Import the Zoom recording into Descript for video editing and studio sound enhancement. Run the Descript agent, then paste the project link below.</p><div><label className="block text-sm font-medium text-dark-text mb-1">Descript Link</label><input ref={descriptLinkRef} type="url" defaultValue={sessionData!.descript_link || ""} placeholder="https://web.descript.com/..." className="w-full border border-dark-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cm-purple focus:border-transparent text-dark-text" /></div><div className="flex items-center gap-3"><button type="button" onClick={() => patchSession({ descript_link: descriptLinkRef.current?.value || "", pipeline_status: { step_3_descript_manual: true } }, "stage3")} disabled={saving} className="flex items-center gap-2 px-4 py-2 bg-cm-purple text-white rounded-lg hover:bg-cm-purple/80 disabled:opacity-50 text-sm font-medium transition-colors">{saving ? <Loader2 size={15} className="animate-spin" /> : <CheckCircle2 size={15} />} Save &amp; Mark Done</button><SaveIndicator statusKey="stage3" /></div></div>);
      ...
      case 20: {
        const qs = sessionData!.questions_saved as { count: number; matched: number; unmatched: number; timestamp: string } | null;
        return (<div className="space-y-3 text-sm text-dark-text"><p className="text-dark-muted">Scans chat highlights for questions asked during the session. Filters out questions that were fully answered live — only unanswered or deferred questions are saved to the participants database for follow-up.</p>{qs ? (<div className="bg-dark-panel2 border border-dark-border rounded-lg px-4 py-3 space-y-1"><div className="flex gap-2"><span className="text-dark-muted w-28 shrink-0">Saved:</span><span className="text-dark-success font-semibold">{qs.count} question{qs.count !== 1 ? "s" : ""}</span></div><div className="flex gap-2"><span className="text-dark-muted w-28 shrink-0">Matched:</span><span>{qs.matched} to participants</span></div>{qs.unmatched > 0 && <div className="flex gap-2"><span className="text-dark-muted w-28 shrink-0">Unmatched:</span><span className="text-dark-warn">{qs.unmatched} — inserted with asked_by = NULL</span></div>}</div>) : <p className="text-dark-muted text-sm">Run this step after Analyze Chat completes.</p>}</div>);
      }
      ...
      case 13: return pipeline.step_14_notion_card ? <div className="text-sm text-dark-text"><span className="text-dark-muted">Notion SM Record ID:</span> <span className="font-mono">{sessionData!.notion_sm_record_id || "Created"}</span></div> : <div className="space-y-2"><p className="text-sm text-dark-muted">Notion card has not been created yet.</p><p className="text-xs text-dark-muted">Requires SM Content Database connection to workspace integration</p></div>;
      ...
      case 16: { const sNum16 = sessionData!.session_number; const nextSNum = sNum16 + 1; const prepUrl = `https://workshop.mastermindshq.business/session/${nextSNum}/prep`; return (<div className="space-y-3">{pipeline.step_15b_tech_output_generated && sessionData!.tech_requirements_output ? <div className="space-y-2 text-sm text-dark-text"><div className="flex items-center gap-2"><CheckCircle2 size={16} className="text-dark-success" /><span>Output generated — prep page built and deployed</span></div><pre className="text-xs font-mono whitespace-pre-wrap bg-dark-bg rounded-lg p-3 max-h-[200px] overflow-y-auto">{sessionData!.tech_requirements_output}</pre><a href={prepUrl} target="_blank" rel="noopener noreferrer" className="text-cm-purple hover:underline flex items-center gap-1.5 text-xs"><ExternalLink size={13} /> /session/{nextSNum}/prep</a><button type="button" onClick={async (e) => { e.preventDefault(); e.stopPropagation(); setRunningStep(16); const r = await runStep("15b"); if (r) setStepResults((p) => ({ ...p, [16]: r.status === "success" ? "success" : "error" })); setRunningStep(null); }} disabled={runningStep === 16} className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-dark-muted border border-dark-border rounded-lg hover:bg-dark-bg disabled:opacity-50 transition-colors">{runningStep === 16 ? <Loader2 size={12} className="animate-spin" /> : <RotateCcw size={12} />} Regenerate</button></div> : <div className="space-y-2"><p className="text-sm text-dark-muted">Tech Requirements output has not been generated yet. Complete step 17 (input) first.</p><p className="text-xs text-dark-muted">Output will be built as a prep web page at <span className="font-mono">/session/{nextSNum}/prep</span> and deployed to Vercel.</p></div>}</div>); }
      case 17: return (<div className="flex items-center gap-2 text-sm text-dark-muted bg-dark-panel2 border border-dark-border rounded-lg px-4 py-3"><span>Intentionally skipped — a prep web page is built instead of a PDF (see Step 18).</span></div>);
      case 18: { const sNum18 = sessionData!.session_number; const nextSNum18 = sNum18 + 1; const prepUrl18 = `https://workshop.mastermindshq.business/session/${nextSNum18}/prep`; const isSent = pipeline.step_15d_tech_wa_sent || sessionData!.tech_requirements_wa_sent; return (<div className="space-y-3"><div className="flex gap-2 text-sm"><span className="text-dark-muted w-28 shrink-0">Prep page:</span><a href={prepUrl18} target="_blank" rel="noopener noreferrer" className="text-cm-purple hover:underline flex items-center gap-1"><ExternalLink size={13} /> /session/{nextSNum18}/prep</a></div>{isSent ? <div className="flex items-center gap-3"><div className="text-sm text-dark-success flex items-center gap-2"><CheckCircle2 size={16} /> Sent to participants</div><button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); patchSession({ pipeline_status: { step_15d_tech_wa_sent: false } }, "stage18"); }} disabled={saving} className="text-xs text-dark-muted hover:text-dark-muted underline transition-colors">undo</button></div> : <button type="button" onClick={async (e) => { e.preventDefault(); e.stopPropagation(); if (testMode) return; setRunningStep(18); const r = await runStep("15d"); if (r) setStepResults((p) => ({ ...p, [18]: r.status === "success" ? "success" : "error" })); setRunningStep(null); }} disabled={testMode || runningStep === 18 || !pipeline.step_15b_tech_output_generated} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${testMode ? "bg-dark-panel2 text-dark-muted cursor-not-allowed" : "bg-cm-purple text-white hover:bg-cm-purple/80 disabled:opacity-50"}`}>{runningStep === 18 ? <Loader2 size={15} className="animate-spin" /> : <Send size={15} />} {testMode ? "Send to Participants (TEST — will not send)" : "Send to Participants"}</button>}<SaveIndicator statusKey="stage18" /></div>); }
      case 21: {
        const ytId = sessionData!.youtube_id;
        const sNum = sessionData!.session_number;
        const workshopGuideUrl = `https://workshop.mastermindshq.business/session/${sNum}/guide`;
        return (
          <div className="space-y-3 text-sm text-dark-text">
            <p className="text-dark-muted">
              Injects the YouTube recording into the workshop guide page using <span className="font-mono text-xs bg-dark-panel2 px-1.5 py-0.5 rounded">StickyVideoPlayer</span>, then builds and deploys to Vercel.
            </p>
            <div className="space-y-2">
              <div className="flex gap-2">
                <span className="text-dark-muted w-36 shrink-0">YouTube ID:</span>
                {ytId ? <span className="font-mono text-cm-purple">{ytId}</span> : <span className="text-dark-warn">Waiting for Stage 2</span>}
              </div>
              <div className="flex gap-2">
                <span className="text-dark-muted w-36 shrink-0">Guide file:</span>
                <span className="font-mono text-xs">session-{sNum}-guide.tsx</span>
              </div>
              <div className="flex gap-2">
                <span className="text-dark-muted w-36 shrink-0">Live page:</span>
                <a href={workshopGuideUrl} target="_blank" rel="noopener noreferrer" className="text-cm-purple hover:underline flex items-center gap-1">
                  <ExternalLink size={13} /> /session/{sNum}/guide
                </a>
              </div>
            </div>
            {pipeline.step_2b_workshop_video && (
              <div className="flex items-center gap-2 text-dark-success bg-dark-success/10 border border-dark-success/30 rounded-lg px-3 py-2">
                <CheckCircle2 size={16} className="shrink-0" />
                <span>Video deployed to workshop guide</span>
              </div>
            )}
          </div>
        );
      ...
      case 23: {
        const sNum23 = sessionData!.session_number;
        // Clips folder path is validated server-side in open-folder API route
        const clipsFolder = `projects/mastermind/cohort-1/sessions/session-${sNum23}/clips/instagram`;
        return (
          <div className="space-y-5 text-sm text-dark-text">
            {/* Section A — Clip Candidates */}
            <div className="space-y-3">
              <label className="block text-sm font-medium text-dark-text">Clip Candidates (7 required)</label>
              <textarea
                ref={instagramClipsRef}
                defaultValue={sessionData!.instagram_clips || ""}
                rows={20}
                className="w-full font-mono text-sm border border-dark-border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cm-purple focus:border-transparent resize-y text-dark-text bg-dark-panel2"
                placeholder={`Timestamp: 00:03:30 - 00:03:53\nSpeaker: Jenny\nQuote: "I rebuilt my year-old website in 5 minutes."\nTier: 1\nWhy: Giant win — dramatic before/after contrast\n\n---\n\nTimestamp: HH:MM:SS - HH:MM:SS\nSpeaker: [name]\nQuote: "[quote]"\nTier: 1 / 2 / 3\nWhy: [one sentence]`}
              />
              <p className="text-xs text-dark-muted">Follow clip-selection-rules.md. Target 7 clips, Tier 1 first. Check VTT for: amazing, incredible, 5 minutes, first time, addicted, took me a year.</p>
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); patchSession({ instagram_clips: instagramClipsRef.current?.value || "" }, "stage23"); }}
                  disabled={saving}
                  className="flex items-center gap-2 px-4 py-2 bg-cm-purple text-white rounded-lg hover:bg-cm-purple/80 disabled:opacity-50 text-sm font-medium transition-colors"
                >
                  {saving ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} Save
                </button>
                <SaveIndicator statusKey="stage23" />
              </div>
            </div>
            {/* Section B — Instagram Clips Approval List */}
            <div className="space-y-3 pt-2 border-t border-dark-border">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-dark-text">Clips in clips/instagram/</span>
                <button
                  type="button"
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); fetchClipList(sNum23); }}
                  disabled={clipListLoading}
                  className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-dark-muted border border-dark-border rounded-lg hover:bg-dark-panel2 disabled:opacity-50 transition-colors"
                >
                  {clipListLoading ? <Loader2 size={11} className="animate-spin" /> : <RotateCcw size={11} />}
                  {clipListLoaded ? "Refresh" : "Load Clips"}
                </button>
              </div>
              {clipListLoading && (
                <div className="flex items-center gap-2 text-xs text-dark-muted py-2">
                  <Loader2 size={13} className="animate-spin" /> Loading clips...
                </div>
              )}
              {!clipListLoading && clipListLoaded && clipList.length === 0 && (
                <p className="text-xs text-dark-muted italic">No clips in clips/instagram/ yet.</p>
              )}
              {!clipListLoading && clipList.length > 0 && (
                <div className="space-y-1.5">
                  {clipList.map((clip) => {
                    const isApproved = clip.status === "queued" || clip.status === "posted";
                    const statusBadge = clip.status === "posted"
                      ? <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-dark-success/10 text-dark-success border border-dark-success/30">Posted</span>
                      : clip.status === "queued"
                      ? <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-dark-warn/10 text-dark-warn border border-dark-warn/30">Queued</span>
                      : <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-dark-panel2 text-dark-muted border border-dark-border">Pending</span>;
                    return (
                      <div key={clip.file} className="flex items-center gap-3 bg-dark-panel2 border border-dark-border rounded-lg px-3 py-2">
                        <input
                          type="checkbox"
                          checked={isApproved}
                          ...
                          onChange={async (e) => {
                            ...
                            try {
                              await fetch("/api/cohorts/wrap-up/approve-clip", {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({ file: clip.file, session: sNum23, approved }),
                              });
                              fetchClipList(sNum23);
                            } catch { /* non-critical */ }
                          }}
                        ...
                        <button
                          ...
                          onClick={async (e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            await fetch("/api/cohorts/wrap-up/open-folder", {
                              method: "POST",
                              headers: { "Content-Type": "application/json" },
                              body: JSON.stringify({ path: clip.path }),
                            });
                          }}
                          className="flex-1 text-left text-xs font-mono text-cm-purple hover:underline truncate"
                        >
                          {clip.file}
                        </button>
                        {statusBadge}
            ...
            <div className="space-y-3 pt-2 border-t border-dark-border">
              <button
                ...
                onClick={async (e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setOpeningClipsFolder(true);
                  setOpenFolderStatus("idle");
                  try {
                    const res = await fetch("/api/cohorts/wrap-up/open-folder", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ path: clipsFolder }),
                    });
                    setOpenFolderStatus(res.ok ? "success" : "error");
                  } catch {
                    setOpenFolderStatus("error");
                  } finally {
                    setTimeout(() => { setOpeningClipsFolder(false); setOpenFolderStatus("idle"); }, 2000);
                  }
                }}
                disabled={openingClipsFolder}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-cm-purple/15 text-cm-purple rounded-lg hover:bg-cm-purple/20 disabled:opacity-50 text-xs font-medium transition-colors"
              >
                {openingClipsFolder ? <Loader2 size={12} className="animate-spin" /> : <FolderOpen size={12} />}
                {openingClipsFolder
                  ? openFolderStatus === "success" ? "Opened" : openFolderStatus === "error" ? "Failed" : "Opening..."
                  : "Open Clips Folder in Finder"}
              </button>
              <div className="bg-dark-panel2 border border-dark-border rounded-lg p-3 text-xs text-dark-muted">
                7 clips per session. Run each through: ffmpeg portrait conversion → Remotion instagram-reel-overlay template → save to clips/instagram/. See templates/instagram-clips-template.json for full workflow.
              </div>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={!!pipeline.step_23_instagram_clips}
                  onChange={async (e) => { e.stopPropagation(); patchSession({ pipeline_status: { step_23_instagram_clips: e.target.checked } }, "stage23-check"); }}
                  disabled={saving}
                  className="rounded border-dark-border"
                />
                <span className="text-xs text-dark-muted">Mark Complete</span>
              </label>
            </div>
          </div>
        );
      ...
      case 24: return (
        <div className="space-y-4">
          <div className="flex items-start gap-2 bg-dark-warn/10 border border-dark-warn/30 rounded-lg px-4 py-3 text-sm text-dark-warn">
            <AlertCircle size={15} className="shrink-0 mt-0.5" />
            <span>Post to Instagram on a Schedule — NOT YET WIRED. This step is a placeholder for future automation. When wired, clips from the instagram folder will be scheduled and posted directly to Instagram via the automation pipeline.</span>
          </div>
          <button
            type="button"
            disabled
            className="flex items-center gap-2 px-4 py-2 bg-dark-panel2 text-dark-muted rounded-lg opacity-50 cursor-not-allowed text-sm font-medium"
          >
            <Send size={15} /> Post to Instagram (Coming Soon)
          </button>
        </div>
      );
