...
interface PostLogEntry {
  ...
  title?: string;
  platform?: string;
  status?: "published" | "draft" | "failed" | "dry-run" | "error";
  publishedAt?: string;
  date?: string;
  url?: string;
  postUrl?: string;
...
export async function GET() {
  ...
  // Affiliate stats
  const postsWithAffiliates = postLog.filter((p) => (p.affiliateLinksCount ?? 0) > 0);
  const totalAffiliateLinks = postsWithAffiliates.reduce((sum, p) => sum + (p.affiliateLinksCount ?? 0), 0);
  const programCounts: Record<string, number> = {};
  for (const p of postsWithAffiliates) {
    for (const prog of p.affiliatePrograms ?? []) {
  ...
  return NextResponse.json({
    ...
    revenueBreakdown,
    affiliateStats: {
      totalLinksInjected: totalAffiliateLinks,
      postsWithAffiliates: postsWithAffiliates.length,
      programUsage: programCounts,
    },
  });
}
