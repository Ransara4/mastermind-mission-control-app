...
const SCRIPTS_CWD = path.join(WS, "projects/mastermind");
...
function getStepConfig(
  ...
  const manualSteps = ["6", "8", "10", "11", "14", "15", "15a"];
  ...
  const configs: Record<string, StepConfig> = {
    "3": {
      command: "node",
      args: buildArgs("scripts/descript-import.js", session),
      pipelineKey: "step_3_descript_manual",
    },
    "1": {
      command: "bash",
    ...
    "2b": {
      command: "node",
      args: buildArgs("scripts/deploy-workshop-video.js", session),
      pipelineKey: "step_2b_workshop_video",
    },
    "4": {
      command: "node",
    ...
    "20": {
      command: "node",
      args: buildArgs("scripts/save-questions.js", session),
      pipelineKey: "step_4c_questions_saved",
    },
    "4b": {
      command: "node",
...
async function wixDeleteItem(itemId: string): Promise<void> {
  ...
  await httpsRequest(
    "DELETE",
    `https://www.wixapis.com/wix-data/v2/items/${itemId}?dataCollectionId=MastermindRecordings`,
    {
      Authorization: token,
