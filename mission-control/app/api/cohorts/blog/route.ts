...
const SCHEMA = `CREATE TABLE IF NOT EXISTS blog_posts (
  ...
  cta_type TEXT DEFAULT 'mastermindvsl',
...
function getDb(readonly = true) {
  ...
  if (!readonly) {
    db.exec(SCHEMA);
    try { db.exec("ALTER TABLE blog_posts ADD COLUMN site_domain TEXT DEFAULT 'mastermindshq.business'"); } catch {}
  }
  return db;
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("cohorts blog GET error:", err);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("cohorts blog POST error:", err);
  ...
  try {
    ...
    if (body.status === "queued") {
      try {
        ...
        if (post) {
          const SITE_BASE = "https://www.mastermindshq.business";
          const blogDir = path.join(WS, "projects/mastermind/blog");
          ...
          const fullContent = content
            ...
            .replace(/\]\(\/mastermindvsl\b/g, `](${SITE_BASE}/mastermindvsl`);
    ...
    console.error("cohorts blog PUT error:", err);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error("cohorts blog DELETE error:", err);
