...
// POST /api/manychat-giveaways/activate
// Activates growth tool widget in ManyChat, sets status = 'active' in DB
export async function POST(req: NextRequest) {
  try {
    ...
    const result = runBrowserScript(["set-widget-status", "--ns", flowNs, "--status", "active"]);
    if (!result.success) {
      logSync(db, rowId, "activate", "error", result.error as string | undefined);
      db.close();
      return NextResponse.json(
    ...
    db.prepare(
      "UPDATE giveaways SET status = 'active', updated_at = datetime('now') WHERE id = ?"
    ).run(rowId);
