...
// POST /api/online-program/questions/generate-answers
export async function POST() {
  try {
    ...
    for (const row of unanswered) {
      try {
        const prompt = `You are ${process.env.USER_FULL_NAME || 'the coach'}, a ${process.env.USER_ROLE || 'business coach'} running a online-program program. Answer this participant question concisely and helpfully (2-4 paragraphs max): ${row.question}`;
