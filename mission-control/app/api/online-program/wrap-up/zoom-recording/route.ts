...
// GET /api/online-program/wrap-up/zoom-recording?session=N&cohort=1
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Match pattern: Online Program - Cohort N, Session M - YYYY-MM-DD.mp4
    let matched: string | null = null;
    if (session) {
      const pattern = `Online Program - Cohort ${cohort}, Session ${session}`;
...
// POST /api/online-program/wrap-up/zoom-recording?session=N — open session folder in Finder
