...
export async function POST(req: NextRequest) {
  try {
    ...
    const post = db.prepare("SELECT id, status FROM blog_posts WHERE id = ?").get(id) as
    ...
    const child = spawn("node", [publisherPath, `--id=${id}`], {
      detached: true,
      stdio: "ignore",
    });
    child.unref();
    ...
    console.error("online-program publish POST error:", err);
